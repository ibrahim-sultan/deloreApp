...
// Admin Dashboard - Get dashboard statistics and data
router.get("/dashboard", adminAuth, async (req, res) => {
  try {
    ...
    // Get all tasks with staff details
    ...
    // Group tasks by staff for easier frontend consumption
    const tasksByStaff = {};
    allTasks.forEach((task) => {
      if (task.assignedTo) {
        const staffId = task.assignedTo._id.toString();
        if (!tasksByStaff[staffId]) {
          tasksByStaff[staffId] = {
            staffMember: task.assignedTo,
            tasks: [],
          };
        }
        tasksByStaff[staffId].tasks.push(task);
      }
    });

    // Group documents by staff (placeholder - assuming documents exist)
    const documentsByStaff = staffMembers.map((staff) => ({
      staffMember: staff,
      documents: [], // This would be populated if you have a Document model
    }));

    // Calculate total hours and payments (placeholder values)
    const totalHoursWorked = allTasks
      .filter((task) => task.hoursSpent)
    ...
    const dashboardData = {
      ...
      staffMembers,
      tasksByStaff: Object.values(tasksByStaff),
      documentsByStaff,
...
router.post(
  "/assign-task",
  adminAuth,
  mapUpload.single("mapAttachment"),
  [
    body("title")
      .trim()
      .isLength({ min: 1 })
...
router.get("/staff-report/:staffId", adminAuth, async (req, res) => {
  try {
    ...
    const tasks = await Task.find({ assignedTo: staffId }).sort({
    ...
    const totalHoursSpent = completedTasks.reduce(
      (sum, task) => sum + (task.hoursSpent || 0),
      0,
    );
    const totalScheduledHours = tasks.reduce(
      (sum, task) => sum + (task.totalHours || 0),
      0,
    );
  ...
  async (req, res) => {
    try {
      ...
      if (!task.clockInTime) {
        return res
          .status(400)
          .json({ message: "Task has not been clocked in yet" });
      }
