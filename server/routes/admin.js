...
router.get('/staff/:staffId', adminAuth, async (req, res) => {
  try {
    ...
    // Get staff documents
    const documents = await Document.find({ uploadedBy: staffId })
      .select('title filename expiryDate createdAt')
      .sort({ createdAt: -1 })
      .lean();

    // Get staff tasks
    const tasks = await Task.find({ assignedTo: staffId })
      .select('title status location totalHours scheduledStartTime scheduledEndTime')
      .sort({ scheduledStartTime: -1 })
      .lean();

    // Get staff payments
    const payments = await Payment.find({ staffMember: staffId })
      .select('amount paymentDate status createdAt')
      .sort({ paymentDate: -1 })
      .lean();
...
// Document Templates - GET all templates
router.get('/document-templates', adminAuth, async (req, res) => {
  try {
    ...
    res.status(500).json({ message: 'Failed to load document templates' });
  }
});

// Document Templates - POST create template
router.post('/document-templates', adminAuth, [
  ...
  try {
    ...
    const existing = await DocumentTemplate.findOne({ title: { $regex: new RegExp(`^${title}const express = require('express');
const { body, validationResult } = require('express-validator');
const Task = require('../models/Task');
const User = require('../models/User');
const Document = require('../models/Document');
...
, 'i') } });
    ...
    const template = new DocumentTemplate({ title, description });
    await template.save();
...
// Document Templates - DELETE template
...
router.delete('/tasks/pending/all', adminAuth, async (req, res) => {
  try {
    // Delete all tasks with status 'assigned' (pending tasks)
    const result = await Task.deleteMany({ status: 'assigned' });
    
    res.json({ 
      message: `Successfully deleted ${result.deletedCount} pending tasks`,
      deletedCount: result.deletedCount
    });
  } catch (error) {
    console.error('Error deleting pending tasks:', error);
    res.status(500).json({ message: 'Failed to delete pending tasks' });
  }
});
